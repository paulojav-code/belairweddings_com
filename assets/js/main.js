...
(function($) {
	...
	// Keyboard shortcuts.
		if (settings.keyboardShortcuts.enabled)
			(function() {
				...
				$('#myModal').on('keypress keyup keydown', settings.excludeSelector, function(event) {
					...
					// Keypress event.
						.on('keydown', function(event) {
							...
							switch (event.keyCode) {
								...
								// Left arrow.
									case 37:
										if($('.modal').hasClass('visible')){
											var id = parseInt(hrefImgVisible.match(/galeria\/(.*)\.jpg/)[1]);
											modal($('.modal'),true);
											setHref(id - 1);
										}else{
								...
								// Page Up.
									case 33:
										$document.scrollLeft($document.scrollLeft() - $window.width() + 200);
										scrolled = true;
										break;
		...
		$wrapper
			...
			.on('click', 'a[href^="#"]', function(event) {
				...
				// Calculate x, y.
					...
					else if (breakpoints.active('<=large')) {

						x = $target.offset().left - (Math.max(0, $window.width() - $target.outerWidth()) / 4);
						y = { scrollLeft: x };

					}
					else {

						x = $target.offset().left - (Math.max(0, $window.width() - $target.outerWidth()) / 6);
						y = { scrollLeft: x };
	...
	//Modal
		...
		.on("click", function(event) {
			if ($(event.target).attr('class') == 'modal visible loaded') {
		...
		function modal($modal, img = false){
	...
	// Gallery.
		$('.gallery')
			...
			.prepend('<div class="modal" tabIndex="-1"><div class="inner"><img src="" /></div><div class="flecha pre"><i class="icon solid fa-angle-left"></i></div><div class="flecha next"><i class="icon solid fa-angle-right"></i></div></div>')
				.find('img')
		...
		function setHref(id) {
			if(id <= 0) {
				id = tamanho_galeria; 
			}else if(id > tamanho_galeria){
				id = 0
			}
			var href = 'assets/img/galeria/'+ (id) +'.jpg';

			setTimeout(function() {

				// Set src.
				$('.gallery .modal img').attr('src', href);
				hrefImgVisible = href;
