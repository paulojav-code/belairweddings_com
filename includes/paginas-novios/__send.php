	...
	if ($recaptcha->score >= 0.0) {
		...
		if(!empty($asistencia)) {
			if(!empty($nombre) || !empty($apellido) || !empty($email) || ($asistencia == 1 && !empty($ceremonia)) ) {
				...
				$mail->Subject    = utf8_decode("Confirmación de Asistencia a Boda de " . $nombre_novies);

				if($asistencia == 1){
					switch($ceremonia){
						case 1: 
							$ceremonia_name = 'Ceremonia Religiosa';
							break;
						case 2: 
							$ceremonia_name = 'Recepción';
							break;
						case 3:
							$ceremonia_name = 'Ceremonia Religiosa y Recepción';
							break;
						default:
							break;
					}
