<?php
    if($_SERVER['SERVER_NAME'] == 'localhost'){
        $url_root = '../../../assets/img/wp/';
    }else{
        $url_root = 'C:/inetpub/wwwroot/belairweddings_com/assets/img/wp/';
    }
    ...
    function cover_image($dir,$cover) {
        ...
        $img = set_imagesize($img);
        $img[4] = $dir.'/cover.jpg';
        $img[5] = $dir.'/mini-small.jpg';
        $img[6] = $dir.'/mini-large.jpg';
        $img_source = create_source($img[0],$img[1]);
    ...
    function galeria_image($dir,$galeria,$id) {
        for ($i=0; $i < count($galeria); $i++) { 
            $img = explode('.',$galeria[$i]['url']);
            $img[0] = '../../assets/img/wp/'.$id.'/'.$galeria[$i]['url'];
            orientation_image($img[0],$img[1]);
            $img = set_imagesize($img);
            $img[4] = $dir.'/'.($i+1).'.jpg';
            $img[5] = $dir.'/'.($i+1).'-m.jpg';
    ...
    if(isset($a_rec['id'])){
        switch($a_rec['id']){
            ...
            case '1':
                ...
                break;
            case '2':
                if(!file_exists($url_root.$a_rec['dir'])){
                    mkdir($url_root.$a_rec['dir'], 0700);
                }
                form_image($url_root.$a_rec['dir'],$a_rec['form']);
                cover_image($url_root.$a_rec['dir'],$a_rec['cover']);
                break;
            case '3':
                if(!file_exists($url_root.$a_rec['dir'])){
                    mkdir($url_root.$a_rec['dir'], 0700);
                };
                galeria_image($url_root.$a_rec['dir'],$a_rec['galeria'],$a_rec['dir']);
                break;
            default:
