<?php
    ...
    if(isset($array_resquest['type'])){
        switch($array_resquest['type']){
            case '0':
                //Request SELECT
                $sql = 'SELECT t0.`id_tipo_ceremonia_desc`,t1.`siglas`,t0.`nombre`,t0.`activo`
						FROM admin_bodas.tipos_ceremonias_desc AS t0 
                        INNER JOIN admin_bodas.`idiomas` AS t1 ON t0.`id_idioma` = t1.`id_idioma`
                        WHERE t0.`activo` = 1 ORDER BY id_tipo_ceremonia_desc;';
            ...
            case '1':
                ...
                $sql = "INSERT INTO admin_bodas.tipos_ceremonias_desc (`id_tipo_ceremonia`,`id_idioma`,`nombre`) VALUE (?,?,?);";
                $stmt = $con -> prepare($sql);
            ...
            case '2':
                //Request UPDATE
                if(isset($array_resquest['id_tipo_ceremonia_desc']) && $array_resquest['id_tipo_ceremonia_desc'] > 0 && $array_resquest['id_tipo_ceremonia_desc'] != ''){
                    ...
                    $sql = "UPDATE admin_bodas.tipos_ceremonias_desc SET".$str_set." WHERE `id_tipo_ceremonia_desc` = ?;";
                    $stmt = $con -> prepare($sql);
                    $array_param[0] .= 'i';
                    $array_param[] = &$array_resquest['id_tipo_ceremonia_desc'];
                    call_user_func_array(array($stmt, "bind_param"), $array_param);
                    $stmt -> execute();
            ...
            case '3':
                //Request DELETE
                if(isset($array_resquest['id_tipo_ceremonia_desc']) && $array_resquest['id_tipo_ceremonia_desc'] > 0 && $array_resquest['id_tipo_ceremonia_desc'] != ''){
                    $sql = "UPDATE admin_bodas.tipos_ceremonias_desc SET `activo` = 0 WHERE `id_tipo_ceremonia_desc` = ?;";
                    $stmt = $con -> prepare($sql);
                    $stmt -> bind_param("i", $array_resquest['id_tipo_ceremonia_desc']);
                    $stmt -> execute();
            ...
            case '4':
                //Request ID
                if(isset($array_resquest['id_tipo_ceremonia_desc']) && $array_resquest['id_tipo_ceremonia_desc'] > 0 && $array_resquest['id_tipo_ceremonia_desc'] != ''){
                    $sql = 'SELECT * FROM admin_bodas.tipos_ceremonias_desc WHERE `activo` = 1 AND `id_tipo_ceremonia_desc` = ?;';
                    $stmt = $con -> prepare($sql);
                    $stmt -> bind_param("i", $array_resquest['id_tipo_ceremonia_desc']);
                    $stmt -> execute();
                    $result = $stmt->get_result();
