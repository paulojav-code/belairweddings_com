<?php
    ...
    if(isset($array_resquest['type'])){
        switch($array_resquest['type']){
            case '0':
                //Request SELECT
                $sql = 'SELECT t0.`id_tipo_ceremonia`,t0.`nombre`,t0.`activo`
						FROM admin_bodas.tipos_ceremonias AS t0 
                        
                        WHERE t0.`activo` = 1 ORDER BY id_tipo_ceremonia;';
            ...
            case '1':
                ...
                $sql = "INSERT INTO admin_bodas.tipos_ceremonias (`nombre`) VALUE (?);";
                $stmt = $con -> prepare($sql);
            ...
            case '2':
                //Request UPDATE
                if(isset($array_resquest['id_tipo_ceremonia']) && $array_resquest['id_tipo_ceremonia'] > 0 && $array_resquest['id_tipo_ceremonia'] != ''){
                    ...
                    $sql = "UPDATE admin_bodas.tipos_ceremonias SET".$str_set." WHERE `id_tipo_ceremonia` = ?;";
                    $stmt = $con -> prepare($sql);
                    $array_param[0] .= 'i';
            ...
            case '3':
                //Request DELETE
                if(isset($array_resquest['id_tipo_ceremonia']) && $array_resquest['id_tipo_ceremonia'] > 0 && $array_resquest['id_tipo_ceremonia'] != ''){
                    $sql = "UPDATE admin_bodas.tipos_ceremonias SET `activo` = 0 WHERE `id_tipo_ceremonia` = ?;";
                    $stmt = $con -> prepare($sql);
                    $stmt -> bind_param("i", $array_resquest['id_tipo_ceremonia']);
                    $stmt -> execute();
            ...
            case '4':
                //Request ID
                if(isset($array_resquest['id_tipo_ceremonia']) && $array_resquest['id_tipo_ceremonia'] > 0 && $array_resquest['id_tipo_ceremonia'] != ''){
                    $sql = 'SELECT * FROM admin_bodas.tipos_ceremonias WHERE `activo` = 1 AND `id_tipo_ceremonia` = ?;';
                    $stmt = $con -> prepare($sql);
                    $stmt -> bind_param("i", $array_resquest['id_tipo_ceremonia']);
                    $stmt -> execute();
                    $result = $stmt->get_result();
