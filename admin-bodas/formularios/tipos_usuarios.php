...
?>
		...
		?>
        <script>
			var tabla = jQuery.parseJSON('{"name":"tipos_usuarios","title":"Tipos Usuarios","primary_key":0,"columns":[{"name":"id_tipo_usuario","data_type":"i","field_type":"1","primary_key":true,"insert":false},{"name":"nombre","data_type":"s","field_type":"1","max_size":100},{"name":"permisos","data_type":"s","field_type":"2"},{"name":"activo","data_type":"i","field_type":"1","insert":false}]}');
			...
			function print_fields(id) {
				...
				fields += '</div>';
				fields += '<div class="col-12"><br><ul class="actions"><li><a class="button primary small">Guardar Nuevo</a></li><li><a class="button small" href="../catalogos/tipos_usuarios.php">Cancelar</a></li></li></ul></div>';
				$('#content').html(fields);
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if(typeof(res['error']) == "undefined"){
							if(res['insert'] > 0){
								print_fields(0);
								alert('Se insertó en tipos_usuarios');
								window.location = "../catalogos/tipos_usuarios.php";
							}else{
								alert('No insertó en tipos_usuarios');
							}
						}else{
			...
			function request_data(id) {
				var dataString = 'request={"type":"4","id_tipo_usuario":"'+id+'"}';
			...
			function request_update() {
				var dataString = 'request={"type":"2","id_tipo_usuario":"'+$('#f_id_tipo_usuario').val()+'",';
					for (let i = 0; i < tabla['columns'].length; i++) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if(typeof(res['error']) == "undefined"){
							if(res['update'] > 0){
								print_fields();
								alert('Se actualizó en tipos_usuarios');
								window.location = "../catalogos/tipos_usuarios.php";
							}else{
								alert('No actualizó en tipos_usuarios');
							}
						}else{
							alert('Error:'+res['error']);
						}
