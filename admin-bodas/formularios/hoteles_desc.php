<?php
	...
	$table_title = 'Descripcion Hoteles';
...
?>
		...
		?>
        <script>
			var tabla = jQuery.parseJSON('{"name":"hoteles_desc","title":"Descripcion Hoteles","primary_key":0,"columns":[{"name":"id_hotel_desc","data_type":"i","field_type":"1","primary_key":true,"insert":false},{"name":"id_hotel","data_type":"i","field_type":"0","primary_key":true,"foreign":{"table_name":"hoteles","key_name":"id_hotel","column_name":"nombre"},"select":false},{"name":"id_idioma","data_type":"i","field_type":"0","primary_key":true,"foreign":{"table_name":"idiomas","key_name":"id_idioma","column_name":"siglas"}},{"name":"nombre","data_type":"s","field_type":"1","max_size":100},{"name":"descripcion","data_type":"s","field_type":"2"},{"name":"detalle_tarifas","data_type":"s","field_type":"2"},{"name":"activo","data_type":"i","field_type":"1","insert":false}]}');
			...
			function print_fields(id) {
				...
				fields += '</div>';
				fields += '<div class="col-12"><br><ul class="actions"><li><a class="button primary small">Guardar Nuevo</a></li><li><a class="button small" href="../catalogos/hoteles_desc.php">Cancelar</a></li></li></ul></div>';
				$('#content').html(fields);
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if(typeof(res['error']) == "undefined"){
							if(res['insert'] > 0){
								print_fields(0);
								alert('Se insertó en hoteles_desc');
								window.location = "../catalogos/hoteles_desc.php";
							}else{
								alert('No insertó en hoteles_desc');
							}
						}else{
			...
			function request_data(id) {
				var dataString = 'request={"type":"4","id_hotel_desc":"'+id+'"}';
			...
			function request_update() {
				var dataString = 'request={"type":"2","id_hotel_desc":"'+$('#f_id_hotel_desc').val()+'",';
					for (let i = 0; i < tabla['columns'].length; i++) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if(typeof(res['error']) == "undefined"){
							if(res['update'] > 0){
								print_fields();
								alert('Se actualizó en hoteles_desc');
								window.location = "../catalogos/hoteles_desc.php";
							}else{
								alert('No actualizó en hoteles_desc');
							}
						}else{
							alert('Error:'+res['error']);
						}
