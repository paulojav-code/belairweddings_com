...
?>
		...
		?>
        <script>
			var tabla = jQuery.parseJSON('{"name":"comentarios","title":"Comentarios","primary_key":0,"columns":[{"name":"id_comentario","data_type":"i","field_type":"1","primary_key":true,"insert":false},{"name":"id_novie","data_type":"i","field_type":"0","primary_key":true,"foreign":{"table_name":"novies","key_name":"id_novie","column_name":"id_novie","concat":["person_1","&39; &39;","conector","&39; &39;","person_2"],"select_name":"novies"}},{"name":"nombre","data_type":"s","field_type":"1","max_size":100},{"name":"mensaje","data_type":"s","field_type":"2"},{"name":"fecha_envio","data_type":"s","field_type":"3"},{"name":"fecha_aprobacion","data_type":"s","field_type":"3"},{"name":"activo","data_type":"i","field_type":"1","insert":false}]}');
			...
			function print_fields(id) {
				...
				fields += '</div>';
				fields += '<div class="col-12"><br><ul class="actions"><li><a class="button primary small">Guardar Nuevo</a></li><li><a class="button small" href="../catalogos/comentarios.php">Cancelar</a></li></li></ul></div>';
				$('#content').html(fields);
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if(typeof(res['error']) == "undefined"){
							if(res['insert'] > 0){
								print_fields(0);
								alert('Se insertó en comentarios');
								window.location = "../catalogos/comentarios.php";
							}else{
								alert('No insertó en comentarios');
							}
						}else{
			...
			function request_data(id) {
				var dataString = 'request={"type":"4","id_comentario":"'+id+'"}';
			...
			function request_update() {
				var dataString = 'request={"type":"2","id_comentario":"'+$('#f_id_comentario').val()+'",';
					for (let i = 0; i < tabla['columns'].length; i++) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if(typeof(res['error']) == "undefined"){
							if(res['update'] > 0){
								print_fields();
								alert('Se actualizó en comentarios');
								window.location = "../catalogos/comentarios.php";
							}else{
								alert('No actualizó en comentarios');
							}
						}else{
							alert('Error:'+res['error']);
						}
