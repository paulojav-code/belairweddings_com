...
?>
		...
		?>
        <script>
			var tabla = jQuery.parseJSON('{"name":"habitaciones","title":"Habitaciones","primary_key":0,"columns":[{"name":"id_habitacion","data_type":"i","field_type":"1","primary_key":true,"insert":false},{"name":"id_hotel","data_type":"i","field_type":"0","foreign":{"table_name":"hoteles","key_name":"id_hotel","column_name":"nombre","select_name":"hotel"}},{"name":"nombre","data_type":"s","field_type":"1","max_size":100},{"name":"carpeta","data_type":"s","field_type":"1","max_size":100},{"name":"mini","data_type":"s","field_type":"1","max_size":100},{"name":"galeria","data_type":"i","field_type":"1"},{"name":"activo","data_type":"i","field_type":"1","insert":false}]}');
			...
			function print_fields(id) {
				...
				fields += '</div>';
				fields += '<div class="col-12"><br><ul class="actions"><li><a class="button primary small">Guardar Nuevo</a></li><li><a class="button small" href="../catalogos/habitaciones.php">Cancelar</a></li></li></ul></div>';
				$('#content').html(fields);
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if(typeof(res['error']) == "undefined"){
							if(res['insert'] > 0){
								print_fields(0);
								alert('Se insertó en habitaciones');
								window.location = "../catalogos/habitaciones.php";
							}else{
								alert('No insertó en habitaciones');
							}
						}else{
			...
			function request_data(id) {
				var dataString = 'request={"type":"4","id_habitacion":"'+id+'"}';
			...
			function request_update() {
				var dataString = 'request={"type":"2","id_habitacion":"'+$('#f_id_habitacion').val()+'",';
					for (let i = 0; i < tabla['columns'].length; i++) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if(typeof(res['error']) == "undefined"){
							if(res['update'] > 0){
								print_fields();
								alert('Se actualizó en habitaciones');
								window.location = "../catalogos/habitaciones.php";
							}else{
								alert('No actualizó en habitaciones');
							}
						}else{
							alert('Error:'+res['error']);
						}
