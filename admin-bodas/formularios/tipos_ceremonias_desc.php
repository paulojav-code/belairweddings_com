<?php
	...
	$table_title = 'Descripcion Tipos Ceremonias';
...
?>
		...
		?>
        <script>
			var tabla = jQuery.parseJSON('{"name":"tipos_ceremonias_desc","title":"Descripcion Tipos Ceremonias","primary_key":0,"columns":[{"name":"id_tipo_ceremonia_desc","data_type":"i","field_type":"1","primary_key":true,"insert":false},{"name":"id_tipo_ceremonia","data_type":"i","field_type":"0","primary_key":true,"foreign":{"table_name":"tipos_ceremonias","key_name":"id_tipo_ceremonia","column_name":"nombre"},"select":false},{"name":"id_idioma","data_type":"i","field_type":"0","primary_key":true,"foreign":{"table_name":"idiomas","key_name":"id_idioma","column_name":"siglas"}},{"name":"nombre","data_type":"s","field_type":"1","max_size":100},{"name":"activo","data_type":"i","field_type":"1","insert":false}]}');
			...
			function print_fields(id) {
				...
				fields += '</div>';
				fields += '<div class="col-12"><br><ul class="actions"><li><a class="button primary small">Guardar Nuevo</a></li><li><a class="button small" href="../catalogos/tipos_ceremonias_desc.php">Cancelar</a></li></li></ul></div>';
				$('#content').html(fields);
			...
			function request_insert() {
                ...
                $.ajax({
                    data: dataString,
                    url: '../request/tipos_ceremonias_desc.php',
                    type: 'POST',
                    success: function(response) {
                        ...
                        if(typeof(res['error']) == "undefined"){
							if(res['insert'] > 0){
								print_fields(0);
								alert('Se insertó en tipos_ceremonias_desc');
								window.location = "../catalogos/tipos_ceremonias_desc.php";
							}else{
								alert('No insertó en tipos_ceremonias_desc');
							}
						}else{
			...
			function request_data(id) {
				var dataString = 'request={"type":"4","id_tipo_ceremonia_desc":"'+id+'"}';
			...
			function request_update() {
				var dataString = 'request={"type":"2","id_tipo_ceremonia_desc":"'+$('#f_id_tipo_ceremonia_desc').val()+'",';
					for (let i = 0; i < tabla['columns'].length; i++) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if(typeof(res['error']) == "undefined"){
							if(res['update'] > 0){
								print_fields();
								alert('Se actualizó en tipos_ceremonias_desc');
								window.location = "../catalogos/tipos_ceremonias_desc.php";
							}else{
								alert('No actualizó en tipos_ceremonias_desc');
							}
						}else{
							alert('Error:'+res['error']);
						}
