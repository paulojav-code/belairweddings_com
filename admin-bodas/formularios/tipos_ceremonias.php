...
?>
		...
		?>
        <script>
			var tabla = jQuery.parseJSON('{"name":"tipos_ceremonias","title":"Tipos Ceremonias","primary_key":0,"columns":[{"name":"id_tipo_ceremonia","data_type":"i","field_type":"1","primary_key":true,"insert":false},{"name":"nombre","data_type":"s","field_type":"1","max_size":100},{"name":"activo","data_type":"i","field_type":"1","insert":false}]}');
			...
			function print_fields(id) {
				...
				fields += '</div>';
				fields += '<div class="col-12"><br><ul class="actions"><li><a class="button primary small">Guardar Nuevo</a></li><li><a class="button small" href="../catalogos/tipos_ceremonias.php">Cancelar</a></li></li></ul></div>';
				$('#content').html(fields);
			...
			function request_insert() {
                ...
                $.ajax({
                    data: dataString,
                    url: '../request/tipos_ceremonias.php',
                    type: 'POST',
                    success: function(response) {
                        ...
                        if(typeof(res['error']) == "undefined"){
							if(res['insert'] > 0){
								print_fields(0);
								alert('Se insertó en tipos_ceremonias');
								window.location = "../catalogos/tipos_ceremonias.php";
							}else{
								alert('No insertó en tipos_ceremonias');
							}
						}else{
			...
			function request_data(id) {
				var dataString = 'request={"type":"4","id_tipo_ceremonia":"'+id+'"}';
			...
			function request_update() {
				var dataString = 'request={"type":"2","id_tipo_ceremonia":"'+$('#f_id_tipo_ceremonia').val()+'",';
					for (let i = 0; i < tabla['columns'].length; i++) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if(typeof(res['error']) == "undefined"){
							if(res['update'] > 0){
								print_fields();
								alert('Se actualizó en tipos_ceremonias');
								window.location = "../catalogos/tipos_ceremonias.php";
							}else{
								alert('No actualizó en tipos_ceremonias');
							}
						}else{
							alert('Error:'+res['error']);
						}
