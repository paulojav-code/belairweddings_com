<?php
	...
	$table_title = 'Descripcion Novies';
...
?>
		...
		?>
        <script>
			var tabla = jQuery.parseJSON('{"name":"novies_desc","title":"Descripcion Novies","primary_key":0,"columns":[{"name":"id_novie_desc","data_type":"i","field_type":"1","primary_key":true,"insert":false},{"name":"id_novie","data_type":"i","field_type":"0","primary_key":true,"foreign":{"table_name":"novies","key_name":"id_novie","column_name":"id_novie","concat":["person_1","&39; &39;","conector","&39; &39;","person_2"],"select_name":"Novies"}},{"name":"id_idioma","data_type":"i","field_type":"0","primary_key":true,"foreign":{"table_name":"idiomas","key_name":"id_idioma","column_name":"siglas"}},{"name":"frase","data_type":"s","field_type":"2"},{"name":"tipo_codigo","data_type":"s","field_type":"1","select_name":"Tipo Codigo","default":"0"},{"name":"codigo_vestimenta","data_type":"s","field_type":"2","select_name":"Codigo Vestimenta"},{"name":"guia_novies","data_type":"s","field_type":"1","max_size":300,"select_name":"Guia Novies"},{"name":"telefono","data_type":"s","field_type":"1","max_size":30},{"name":"bitly","data_type":"s","field_type":"1","max_size":300},{"name":"activo","data_type":"i","field_type":"1","insert":false}]}');
			...
			function print_fields(id) {
				...
				fields += '</div>';
				fields += '<div class="col-12"><br><ul class="actions"><li><a class="button primary small">Guardar Nuevo</a></li><li><a class="button small" href="../catalogos/novies_desc.php">Cancelar</a></li></li></ul></div>';
				$('#content').html(fields);
			...
			function print_select(res,col,id) {
				...
				for (let i = 0; i < res.length; i++) {
					if(Array.isArray(col['foreign']['column_name'])){
						...
						if(col['foreign']['column_name'] == 'id_novie'){
							select += '<option value="'+res[i][col['foreign']['key_name']]+'"'+( res[i][col['foreign']['key_name']]==id ? ' selected' : '')+'>'+res[i][col['foreign']['column_name']]+' - '+res[i]['Novie 1']+' '+res[i]['conector']+' '+res[i]['Novie 2']+'</option>';
						}else{
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if(typeof(res['error']) == "undefined"){
							if(res['insert'] > 0){
								print_fields(0);
								alert('Se insertó en novies_desc');
								//window.location = "../catalogos/novies_desc.php";
								window.location = "../formularios/ceremonias.php";
							}else{
								alert('No insertó en novies_desc');
							}
						}else{
			...
			function request_data(id) {
				var dataString = 'request={"type":"4","id_novie_desc":"'+id+'"}';
			...
			function request_update() {
				var dataString = 'request={"type":"2","id_novie_desc":"'+$('#f_id_novie_desc').val()+'",';
					for (let i = 0; i < tabla['columns'].length; i++) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if(typeof(res['error']) == "undefined"){
							if(res['update'] > 0){
								print_fields();
								alert('Se actualizó en novies_desc');
								window.location = "../catalogos/novies_desc.php";
							}else{
								alert('No actualizó en novies_desc');
							}
						}else{
							alert('Error:'+res['error']);
						}
