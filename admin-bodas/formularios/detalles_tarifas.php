...
?>
		...
		?>
        <script>
			var tabla = jQuery.parseJSON('{"name":"detalles_tarifas","title":"Detalles Tarifas","primary_key":0,"columns":[{"name":"id_detalle_tarifa","data_type":"i","field_type":"1","primary_key":true,"insert":false},{"name":"id_novie","data_type":"i","field_type":"0","primary_key":true,"foreign":{"table_name":"novies","key_name":"id_novie","column_name":"id_novie","concat":["person_1","&39; &39;","conector","&39; &39;","person_2"],"select_name":"Novies"}},{"name":"id_idioma","data_type":"i","field_type":"0","foreign":{"table_name":"idiomas","key_name":"id_idioma","column_name":"siglas"}},{"name":"edades","data_type":"s","field_type":"2","default":"(*) Persona Extra: De 13 a\u00f1os en adelante. (**) Menor: De 3 a 12 a\u00f1os."},{"name":"limite","data_type":"s","field_type":"2","default":"<b style=\\\"font-size:1.2em\\\">En caso de realizar la reservación a través de la página web, el pago deberá ser total</b> <br><br>Fecha l\u00edmite de pago del bloqueo el d\u00eda 1 de marzo de 2021. Fecha l\u00edmite de pago el d\u00eda 5 de mayo de 2021. Las tarifas arriba mencionadas son en pesos mexicanos, por persona por noche en plan todo incluido. La capacidad m\u00e1xima en las habitaciones es de 2 adultos y 2 menores de 12 a\u00f1os o 3 adultos sin menores."},{"name":"cancelacion","data_type":"s","field_type":"2"},{"name":"activo","data_type":"i","field_type":"1","insert":false}]}');
			...
			function print_fields(id) {
				...
				fields += '</div>';
				fields += '<div class="col-12"><br><ul class="actions"><li><a class="button primary small">Guardar Nuevo</a></li><li><a class="button small" href="../catalogos/detalles_tarifas.php">Cancelar</a></li></li></ul></div>';
				$('#content').html(fields);
			...
			function request_insert() {
                var dataString = 'request={"type":"1",';
					for (let i = 0; i < tabla['columns'].length; i++) {
						if(typeof(tabla['columns'][i]['insert']) == "undefined" || tabla['columns'][i]['insert']){
							dataString += '"'+tabla['columns'][i]['name']+'":"'+encodeURIComponent($('#f_'+tabla['columns'][i]['name']).val().replace(/"/gi, '\\"'))+'",';
						}
					}
                ...
                $.ajax({
                    data: dataString,
                    url: '../request/detalles_tarifas.php',
                    type: 'POST',
                    success: function(response) {
                        ...
                        if(typeof(res['error']) == "undefined"){
							if(res['insert'] > 0){
								print_fields(0);
								alert('Se insertó en detalles_tarifas');
								window.location = "../catalogos/detalles_tarifas.php";
							}else{
								alert('No insertó en detalles_tarifas');
							}
						}else{
			...
			function request_data(id) {
				var dataString = 'request={"type":"4","id_detalle_tarifa":"'+id+'"}';
			...
			function request_update() {
				var dataString = 'request={"type":"2","id_detalle_tarifa":"'+$('#f_id_detalle_tarifa').val()+'",';
					for (let i = 0; i < tabla['columns'].length; i++) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if(typeof(res['error']) == "undefined"){
							if(res['update'] > 0){
								print_fields();
								alert('Se actualizó en detalles_tarifas');
								window.location = "../catalogos/detalles_tarifas.php";
							}else{
								alert('No actualizó en detalles_tarifas');
							}
						}else{
							alert('Error:'+res['error']);
						}
