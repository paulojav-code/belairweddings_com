...
?>
		...
		?>
        <script>
			var tabla = jQuery.parseJSON('{"name":"novies_token","title":"Token Novies","primary_key":0,"columns":[{"name":"id_novie_token","data_type":"i","field_type":"1","primary_key":true,"insert":false},{"name":"id_novie","data_type":"i","field_type":"0","primary_key":true,"foreign":{"table_name":"novies","key_name":"id_novie","column_name":"id_novie","concat":["person_1","&39; &39;","conector","&39; &39;","person_2"],"select_name":"Novies"}},{"name":"token","data_type":"s","field_type":"2"},{"name":"check_in","data_type":"s","field_type":"3","select_name":"Check In"},{"name":"check_out","data_type":"s","field_type":"3","select_name":"Check Out"},{"name":"activo","data_type":"i","field_type":"1","insert":false}]}');
			...
			function print_fields(id) {
				...
				fields += '</div>';
				fields += '<div class="col-12"><br><ul class="actions"><li><a class="button primary small">Guardar Nuevo</a></li><li><a class="button small" href="../catalogos/novies_token.php">Cancelar</a></li></li></ul></div>';
				$('#content').html(fields);
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if(typeof(res['error']) == "undefined"){
							if(res['insert'] > 0){
								print_fields(0);
								alert('Se insertó en novies_token');
								window.location = "../catalogos/novies_token.php";
							}else{
								alert('No insertó en novies_token');
							}
						}else{
			...
			function request_data(id) {
				var dataString = 'request={"type":"4","id_novie_token":"'+id+'"}';
			...
			function request_update() {
				var dataString = 'request={"type":"2","id_novie_token":"'+$('#f_id_novie_token').val()+'",';
					for (let i = 0; i < tabla['columns'].length; i++) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if(typeof(res['error']) == "undefined"){
							if(res['update'] > 0){
								print_fields();
								alert('Se actualizó en novies_token');
								window.location = "../catalogos/novies_token.php";
							}else{
								alert('No actualizó en novies_token');
							}
						}else{
							alert('Error:'+res['error']);
						}
