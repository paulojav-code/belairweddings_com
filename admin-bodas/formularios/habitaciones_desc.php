<?php
	...
	$table_title = 'Descripcion Habitaciones';
...
?>
		...
		?>
        <script>
			var tabla = jQuery.parseJSON('{"name":"habitaciones_desc","title":"Descripcion Habitaciones","primary_key":0,"columns":[{"name":"id_habitacion_desc","data_type":"i","field_type":"1","primary_key":true,"insert":false},{"name":"id_habitacion","data_type":"i","field_type":"0","primary_key":true,"foreign":{"table_name":"habitaciones","key_name":"id_habitacion","column_name":"id_habitacion"},"select":false},{"name":"id_idioma","data_type":"i","field_type":"0","primary_key":true,"foreign":{"table_name":"idiomas","key_name":"id_idioma","column_name":"siglas"}},{"name":"nombre","data_type":"s","field_type":"1","max_size":100},{"name":"descripcion","data_type":"s","field_type":"2"},{"name":"servicios","data_type":"s","field_type":"2"},{"name":"vista","data_type":"s","field_type":"1","max_size":100},{"name":"ocupacion","data_type":"s","field_type":"1","max_size":20},{"name":"activo","data_type":"i","field_type":"1","insert":false}]}');
			...
			function print_fields(id) {
				...
				fields += '</div>';
				fields += '<div class="col-12"><br><ul class="actions"><li><a class="button primary small">Guardar Nuevo</a></li><li><a class="button small" href="../catalogos/habitaciones_desc.php">Cancelar</a></li></li></ul></div>';
				$('#content').html(fields);
			...
			function print_select(res,col,id) {
				...
				for (let i = 0; i < res.length; i++) {
					if(Array.isArray(col['foreign']['column_name'])){
						...
						if(col['foreign']['column_name'] == 'id_habitacion'){
							select += '<option value="'+res[i][col['foreign']['key_name']]+'"'+( res[i][col['foreign']['key_name']]==id ? ' selected' : '')+'>'+res[i]['hotel']+' - '+res[i]['nombre']+'</option>';
						}else{
							select += '<option value="'+res[i][col['foreign']['key_name']]+'"'+( res[i][col['foreign']['key_name']]==id ? ' selected' : '')+'>'+res[i][col['foreign']['column_name']]+'</option>';
						}
					}
			...
			function request_insert() {
                ...
                $.ajax({
                    data: dataString,
                    url: '../request/habitaciones_desc.php',
                    type: 'POST',
                    success: function(response) {
                        ...
                        if(typeof(res['error']) == "undefined"){
							if(res['insert'] > 0){
								print_fields(0);
								alert('Se insertó en habitaciones_desc');
								window.location = "../catalogos/habitaciones_desc.php";
							}else{
								alert('No insertó en habitaciones_desc');
							}
						}else{
			...
			function request_data(id) {
				var dataString = 'request={"type":"4","id_habitacion_desc":"'+id+'"}';
			...
			function request_update() {
				var dataString = 'request={"type":"2","id_habitacion_desc":"'+$('#f_id_habitacion_desc').val()+'",';
					for (let i = 0; i < tabla['columns'].length; i++) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if(typeof(res['error']) == "undefined"){
							if(res['update'] > 0){
								print_fields();
								alert('Se actualizó en habitaciones_desc');
								window.location = "../catalogos/habitaciones_desc.php";
							}else{
								alert('No actualizó en habitaciones_desc');
							}
						}else{
							alert('Error:'+res['error']);
						}
