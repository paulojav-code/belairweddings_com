...
if (count($array_resquest) > 0) {
	include_once('../include/php/creador-wp.php');
	$s = [];
	$p = [''];
	foreach ($array_response as $i => $a) {
		$p[0] .= 'i';
		$s[] = '?';
		$p[] = &$array_response[$i]['id_novie']; // &$array_response[$i]['id_movie] es una variable o detectada como una variable ya que bind_param solo funciona con variables 
	};
	$sql = 'UPDATE novies SET activo = 1 WHERE id_novie IN(' . implode(",", $s) . ');';
	query($con, $sql, $p);
}
...
<div id="page-wrapper">
	...
	<script>
		...
		function print_tabla(res) {
			...
			const COLUMNS = {
				'id_novie': function(col) {
					return '<th width="20%">' + col + '</th>';
				},
				'activo': function(col) {
					return '<th width="10%">' + col + '</th>';
				},
			};
		...
		$('#publicar .button').on('click', async function() {
			...
			request += ']';
			let files_names = request.replace(/[\[\]]/g,"")
			let url = "../apis/guias/index.php"
			let json = {
				id: files_names,
				action: 'post',
			};
			window.location = "index.php?request="+request;


			async function request_api({url,json}){
