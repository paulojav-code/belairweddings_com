        ...
        <script>
            ...
            function print_tabla(res) {
				...
				for (let i = 0; i < data_res.length; i++) {
					...
					if(data_res[i].fecha.split('-')[0] > year || (data_res[i].fecha.split('-')[0] == year && data_res[i].fecha.split('-')[1] > month)){
						tbody += '<tr>';
						for (let j = 0; j < key_res.length; j++) {
							if(key_res[j] == 'id_novie'){
								tbody += '<td><a href="../formularios/novies.php?id='+data_res[i][key_res[j]]+'">' + data_res[i][key_res[j]] + '</a></td>';
							}else{
